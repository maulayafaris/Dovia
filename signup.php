<!-- version 1.0 -->
<!-- CSS Login file powered by Raj78 -->
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <title>Sign Up Page</title>
    <!-- Offline Bootstrap CSS -->
    <link rel="stylesheet" href="resources/css/bootstrap.css">
    <link rel="stylesheet" href="resources/css/bootstrap.min.css"> 
    <link rel="stylesheet" href="resources/css/login.css">

    <!-- Javascript -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>

    <!-- Mie Style -->
    <link rel="stylesheet" href="resources/floating-labels.css">
    <link rel="stylesheet" href="resources/css/univstyle.css">
    
</head>
<?php
include "connect.php";

if (isset($_POST['signup']) && $_POST['signup'] == "SignUp") {
  $fullname = $_POST['nama'];
  $no_telp = $_POST['no_telp'];
  $email = $_POST['email'];
  $passwd = $_POST['password'];

  $query = mysqli_query($connect, "INSERT INTO user_data (nama, email, password, no_telp) VALUE ('$fullname', '$email', '$passwd', '$no_telp')");
  if ($query) {
    header('location:login.php');
  }
}

?>
<body>
<!------ Include the above in your HEAD tag ---------->
<div class="wrapper fadeInDown">
  <div id="formContent">
    <!-- Tabs Titles -->

    <!-- Icon -->
    <div class="fadeIn first">
      <img src="develop-img/logo-lg.png" id="icon" alt="User Icon" />
    </div>

    <!-- Login Form -->
    <form action="" method="post">
      <h3>Sign Up</h3>
      <input type="text" name="nama" placeholder="Nama Lengkap">
      <input type="text" name="no_telp" placeholder="Nomer telephone">
      <input type="text" class="fadeIn second" name="email" placeholder="Email">
      <input type="password" class="fadeIn third" name="password" placeholder="Password">
      <input type="submit" class="fadeIn fourth" name="signup" value="SignUp">
    </form>
    
  </div>
</div>
</body>
</html>